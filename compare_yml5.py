...
# ============================================================
# 3. RECURSIVE COMPARISON ENGINE (now expands missing/extra)
# ============================================================

def expand_container(container, path, typ):
    """
    For missing/extra, if the value is a dict/list, recursively generate
    separate entries for each child instead of reporting the whole container.
    """
    items = []
    if isinstance(container, dict):
        for k, v in container.items():
            ...
            if isinstance(v, (dict, list)):
                ...
                items.extend(expand_container(v, new_path, typ))
            else:
                items.append({'path': new_path, 'type': typ, 'ref_value': v if typ=='missing' else None,
                               'other_value': v if typ=='extra' else None})
        ...
        # scalar
        items.append({'path': path, 'type': typ, 'ref_value': container if typ=='missing' else None,
                       'other_value': container if typ=='extra' else None})
    return items
...
def compare_data(ref, other, path=""):
    ...
    if isinstance(ref, dict) and isinstance(other, dict):
        ...
        # Missing keys: expand the value if it is a container
        for key in ref_keys - other_keys:
            val = ref[key]
            new_path = f"{path}.{key}" if path else key
            if isinstance(val, (dict, list)):
                diffs.extend(expand_container(val, new_path, 'missing'))
            else:
                diffs.append({'path': new_path, 'type': 'missing', 'ref_value': val, 'other_value': None})

        # Extra keys: expand the value if it is a container
        for key in other_keys - ref_keys:
            val = other[key]
            new_path = f"{path}.{key}" if path else key
            if isinstance(val, (dict, list)):
                diffs.extend(expand_container(val, new_path, 'extra'))
            else:
                diffs.append({'path': new_path, 'type': 'extra', 'ref_value': None, 'other_value': val})
    ...
    if isinstance(ref, list) and isinstance(other, list):
        ...
        # Extra elements in ref (missing in other)
        ...
        # Extra elements in other (extra in ref)
